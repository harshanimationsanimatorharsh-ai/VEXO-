...
class PlaybackController(private val context: Context) {
    ...
    /** Rebuild playlist from EditorState — respects trimStart/trimEnd */
    ...
    fun seekToMs(positionMs: Long, state: EditorState) {
        ...
        state.clips.forEachIndexed { index, clip ->
            ...
            if (positionMs <= end) {
                val posInClip = (positionMs - offset).coerceIn(0, clip.trimmedDurationMs)
                p.seekTo(index, clip.trimStartMs + posInClip)
                return
            }
            offset = end
        }
    ...
    /** Current playhead position in timeline ms */
    fun currentPositionMs(state: EditorState): Long {
        ...
        state.clips.forEachIndexed { i, clip ->
            if (i == idx) {
                val posInClip = (p.currentPosition - clip.trimStartMs).coerceAtLeast(0)
