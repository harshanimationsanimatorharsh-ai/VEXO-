...
class VEditorViewModel : ViewModel() {
    ...
    fun undo() { val prev = undoRedo.undo(current()) ?: return; _project.value = prev }
    fun redo() { val next = undoRedo.redo(current()) ?: return; _project.value = next }
...
class VTimeline @JvmOverloads constructor(
    ...
    fun setProject(p: VProject) { project = p; invalidate() }
    fun setSelectedClipId(id: String?) { selectedClipId = id; invalidate() }
    fun setPlayheadMs(ms: Long) { playheadMs = ms; invalidate() }
            ...
            MotionEvent.ACTION_DOWN -> {
                ...
                for (clip in project.clips) {
                    ...
                    if (y in CLIP_TOP..(CLIP_TOP + CLIP_HEIGHT)) {
                        when {
                            x in startX..(startX + HANDLE_WIDTH) -> { dragMode = DragMode.LEFT; dragClipId = clip.id }
                            x in (endX - HANDLE_WIDTH)..endX -> { dragMode = DragMode.RIGHT; dragClipId = clip.id }
                            x in startX..endX -> { dragMode = DragMode.CLIP; dragClipId = clip.id; listener?.onClipSelected(clip.id) }
                        }
                ...
                val phX = msToX(playheadMs)
                if (dragMode == DragMode.NONE && Math.abs(x - phX) < 30f) dragMode = DragMode.HEAD
                lastTouchX = x
            }
...
class EditorActivity : AppCompatActivity() {
    ...
    private fun setupTimeline() {
        timelineView.listener = object : VTimeline.Listener {
            override fun onClipSelected(clipId: String) { viewModel.selectClip(clipId) }
            override fun onSeek(positionMs: Long) { playback.seekTo(positionMs) }
    ...
    private fun observeViewModel() {
        ...
        viewModel.selectedClipId.observe(this) { id ->
            ...
            tvClipInfo.text = if (clip != null)
                "Clip ${clip.order + 1} | ${clip.trimmedDurationMs / 1000}s | ${clip.speed}x"
